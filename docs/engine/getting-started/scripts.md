...
?> Why multiplying the angle by `dt` ? `dt` represents the delta since the last frame. In order to achieve frame-rate indipendent and smooth animations, we need to multiply the angle by `dt`.
